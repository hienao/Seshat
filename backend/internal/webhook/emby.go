...
func NewEmbyProvider() Provider {
	return &embyProvider{
		definition: AppDefinition{
			Code:             "emby",
			Name:             "Emby",
			Description:      "Emby 媒体库、播放、用户与系统事件",
			AuthMode:         "endpoint_url",
			DefaultEventType: DefaultEventType,
			EventTypes:       embyEventTypes(),
		},
		aliases: map[string]string{
...
func embyEventTypes() []EventTypeDefinition {
...
func (p *embyProvider) Code() string                            { return p.definition.Code }
func (p *embyProvider) Definition() AppDefinition               { return p.definition }
func (p *embyProvider) Verify(_ string, _ IncomingRequest) bool { return true }
func (p *embyProvider) DetectType(request IncomingRequest) string {
...
func (p *embyProvider) MapType(sourceEventType string) string {
...
func (p *embyProvider) ExternalEventID(request IncomingRequest) string {
	payload := decodePayload(request.Body)
	return firstString(payload, "EventId", "event_id", "NotificationId", "notification_id", "IdempotencyKey")
}
func (p *embyProvider) Normalize(eventType string, request IncomingRequest) Presentation {
	payload := decodePayload(request.Body)
	...
	if category == "media" || category == "playback" {
		media = normalizeEmbyMedia(payload, item)
	}
	if category == "playback" || category == "security" || category == "user" {
		actor = normalizeEmbyActor(payload, user, session)
	}
	if category == "playback" {
		playback = normalizeEmbyPlayback(payload, playState, media)
	}
	if category == "system" {
		system = normalizeEmbySystem(eventType, payload, task)
	}
...
func normalizeEmbyMedia(payload, item map[string]interface{}) map[string]interface{} {
	...
	putString(media, "season", firstStringFrom(values, "ParentIndexNumber", "SeasonNumber", "SeasonNumber00", "season_number"))
	putString(media, "episode", firstStringFrom(values, "IndexNumber", "EpisodeNumber", "EpisodeNumber00", "episode_number"))
	...
	if providers := normalizeEmbyProviderIDs(payload, item); len(providers) > 0 {
		media["provider_ids"] = providers
	}
	if video := normalizeEmbyVideo(payload, item); len(video) > 0 {
		media["video"] = video
	}
...
func normalizeEmbyActor(payload, user, session map[string]interface{}) map[string]interface{} {
	...
	putString(actor, "client", firstStringFrom(values, "Client", "ClientName", "AppName", "client_name"))
	putString(actor, "remote_ip", firstStringFrom(values, "RemoteEndPoint", "RemoteEndpoint", "RemoteAddress", "remote_ip"))
	return actor
}

func normalizeEmbyProviderIDs(payload, item map[string]interface{}) map[string]interface{} {
...
func normalizeEmbyVideo(payload, item map[string]interface{}) map[string]interface{} {
	...
	for _, container := range []map[string]interface{}{item, payload} {
		...
		for _, candidate := range items {
			...
			putEmbyStringIfEmpty(video, "title", firstStringFrom([]map[string]interface{}{stream}, "DisplayTitle", "Title"))
			putEmbyStringIfEmpty(video, "codec", firstStringFrom([]map[string]interface{}{stream}, "Codec"))
			putEmbyStringIfEmpty(video, "profile", firstStringFrom([]map[string]interface{}{stream}, "Profile"))
			putEmbyStringIfEmpty(video, "width", firstStringFrom([]map[string]interface{}{stream}, "Width"))
			putEmbyStringIfEmpty(video, "height", firstStringFrom([]map[string]interface{}{stream}, "Height"))
			putEmbyStringIfEmpty(video, "range", firstStringFrom([]map[string]interface{}{stream}, "VideoRange", "VideoRangeType"))
			break
		}
	}
...
func putEmbyStringIfEmpty(target map[string]interface{}, key, value string) {
	if stringValue(target, key) == "" {
		putString(target, key, value)
	}
}

func normalizeEmbyPlayback(payload, playState, media map[string]interface{}) map[string]interface{} {
	return normalizePlaybackValues([]map[string]interface{}{playState, payload}, media)
}

func normalizeEmbySystem(eventType string, payload, task map[string]interface{}) map[string]interface{} {
