...
export function PublicEventPage() {
  ...
  useEffect(() => {
    ...
    document.title = event.data?.title || 'Webhook 消息'
    return () => {
      robots.remove()
      referrer.remove()
  ...
  return (
    <main className="min-h-screen bg-neutral-50 px-4 py-10 dark:bg-neutral-950 sm:px-6 sm:py-16">
      <div className="mx-auto max-w-4xl">
        {event.isPending ? <div className="grid min-h-[50vh] place-items-center"><Spinner className="size-8" aria-label="正在加载消息" /></div>
          : event.error || !event.data ? <div className="grid min-h-[50vh] place-items-center"><ErrorState message={errorMessage(event.error, '消息不存在或链接已失效')} /></div>
