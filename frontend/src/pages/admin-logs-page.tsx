...
function formatHeaders(value: string) { try { return JSON.stringify(JSON.parse(value), null, 2) } catch { return value } }
...
export function AdminLogsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7 px-4 py-10 sm:px-6">
      ...
      {selectedId !== null && <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={() => setSelectedId(null)}><aside className="h-full w-full max-w-2xl overflow-y-auto bg-white p-6 shadow-2xl dark:bg-neutral-950" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase tracking-widest text-emerald-700">Log Detail</p><h2 className="mt-1 text-xl font-bold">接口调用详情</h2></div><AppButton size="sm" variant="outline" onClick={() => setSelectedId(null)}>关闭</AppButton></div>{detail.isPending ? <p className="mt-8 text-sm text-neutral-500">正在加载详情…</p> : detail.error ? <div className="mt-8"><Message variant="error" title={errorMessage(detail.error)} /></div> : detail.data ? <div className="mt-6 space-y-5"><div className="grid grid-cols-2 gap-3 text-sm">{[['时间', formatDate(detail.data.occurred_at)], ['状态', String(detail.data.status_code)], ['耗时', `${detail.data.latency_ms} ms`], ['Request ID', detail.data.request_id], ['用户', detail.data.username || '-'], ['App', detail.data.app_code || '-'], ['接入实例', detail.data.integration_id ? `#${detail.data.integration_id}` : '-'], ['消息 ID', detail.data.event_id ? `#${detail.data.event_id}` : '-'], ['请求大小', `${detail.data.request_bytes} bytes`], ['响应大小', `${detail.data.response_bytes} bytes`], ['错误代码', detail.data.error_code || '-'], ['IP', detail.data.client_ip || '-'], ['User-Agent', detail.data.user_agent || '-']].map(([label, value]) => <div key={label} className="rounded-lg bg-neutral-50 p-3 dark:bg-neutral-900"><p className="text-xs text-neutral-500">{label}</p><p className="mt-1 break-all text-sm font-medium">{value}</p></div>)}</div>{detail.data.error_message && <div className="rounded-lg bg-red-50 p-4 text-sm text-red-800 dark:bg-red-950/30 dark:text-red-200"><div className="flex gap-2"><AlertCircle size={17} />{detail.data.error_message}</div></div>}{detail.data.request_headers && <section><h3 className="mb-2 text-sm font-semibold">请求头（已脱敏）</h3><pre className="max-h-64 overflow-auto rounded-xl bg-neutral-950 p-4 text-xs leading-6 text-emerald-100">{formatHeaders(detail.data.request_headers)}</pre></section>}{detail.data.request_body && <section><h3 className="mb-2 text-sm font-semibold">请求正文{detail.data.request_body.includes('...[truncated') ? '（已截断）' : ''}</h3><pre className="max-h-96 overflow-auto whitespace-pre-wrap break-all rounded-xl bg-neutral-950 p-4 text-xs leading-6 text-emerald-100">{detail.data.request_body}</pre></section>}<details><summary className="cursor-pointer text-sm font-medium text-neutral-500">查看完整结构化日志</summary><pre className="mt-2 overflow-auto rounded-xl bg-neutral-950 p-4 text-xs leading-6 text-emerald-100">{JSON.stringify(detail.data, null, 2)}</pre></details></div> : null}</aside></div>}
    </div>
  )
}
