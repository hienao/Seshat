...
export function EmbyEventCard(props: EventCardProps) {
  ...
  return <EventCardView {...props} model={embyModel(props, t)} />
}

function embyModel({ event }: EventCardProps, t: TFunction): EventCardModel {
  ...
  return {
    ...
    metadataAttribution: text(media.metadata_source) === 'emby' ? t('events.metadataBy', { provider: 'Emby' }) : text(media.metadata_source) === 'tmdb' ? t('events.metadataBy', { provider: 'TMDB' }) : '', rawPreview: '',
  }
}
