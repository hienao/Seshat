...
export const barkChannelAdapter: NotificationChannelAdapter = {
  ...
  fieldsFromChannel: (channel) => ({ baseUrl: String(channel.config.base_url ?? 'https://api.day.app'), deviceKey: '', group: String(channel.config.group ?? 'Seshat'), sound: String(channel.config.sound ?? '') }),
  toPayload: (common, fields) => {
    const deviceKey = textField(fields, 'deviceKey').trim()
    return channelPayload(common, { base_url: textField(fields, 'baseUrl').trim(), group: textField(fields, 'group').trim(), sound: textField(fields, 'sound').trim() }, deviceKey ? { device_key: deviceKey } : undefined)
  },
  Form: ({ fields, keepsExistingCredential, update }) => (
    <>
      ...
      <label className="block space-y-2 text-sm font-medium">
        <span>{secretLabel('Device Key', keepsExistingCredential)}</span>
        <Input type="password" value={textField(fields, 'deviceKey')} onChange={(event) => update('deviceKey', event.target.value)} required={!keepsExistingCredential} />
      </label>
