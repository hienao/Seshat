...
import { booleanField, channelPayload, secretLabel, textField } from '../types'

export const telegramChannelAdapter: NotificationChannelAdapter = {
  type: 'telegram',
  label: 'Telegram',
  defaultFields: () => ({ botToken: '', chatId: '', threadId: '', silent: false }),
  fieldsFromChannel: (channel) => ({ botToken: '', chatId: String(channel.config.chat_id ?? ''), threadId: String(channel.config.message_thread_id ?? ''), silent: Boolean(channel.config.silent) }),
  toPayload: (common, fields) => {
    const token = textField(fields, 'botToken').trim()
    const threadId = textField(fields, 'threadId').trim()
    return channelPayload(common, { chat_id: textField(fields, 'chatId').trim(), silent: booleanField(fields, 'silent'), ...(threadId ? { message_thread_id: Number(threadId) } : {}) }, token ? { bot_token: token } : undefined)
  },
  Form: ({ fields, keepsExistingCredential, update }) => (
    <>
      <label className="block space-y-2 text-sm font-medium">
        <span>{secretLabel('Bot Token', keepsExistingCredential)}</span>
        <Input type="password" value={textField(fields, 'botToken')} onChange={(event) => update('botToken', event.target.value)} required={!keepsExistingCredential} />
      </label>
      <label className="block space-y-2 text-sm font-medium">
        <span>Chat ID</span>
