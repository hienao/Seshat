...
- Webhook 事件只持久化原始消息及去重、路由所需的事件元数据；列表、详情和首次推送会按需生成标准化展示
- Jellyfin/Emby 在展示时按 Item ID 查询对应媒体服务器，并短期复用媒体缓存；可选配置 TMDB API 密钥作为补充和回退
...
“系统管理 → 系统设置”中的“对外访问地址”为可选项。保存用户可访问的地址（例如 `https://seshat.example.com`）后，实际推送会附加不可猜测的公开详情链接；未配置时推送仍会正常发送，只是不附带该链接。公开详情页只返回按需生成的标准化展示，不包含原始 Webhook 内容、接入实例、推送状态或日志信息；链接本身具有访问权限，请勿公开转发。
